...
○ D) Fourth option to complete multiple choice
    ...
    with tab3:
        ...
        with col1:
            st.metric("📁 Documents Uploaded", len(st.session_state.uploaded_files) if st.session_state.uploaded_files else 0)

        with col2:
            st.metric("❓ Questions Asked", len(st.session_state.get('chat_history', [])))

        with col3:
            st.metric("🎯 Quizzes Taken", len(st.session_state.get('quiz_history', [])))
