...
Main Streamlit Application with Fallback Support
"""
...
from modules.utils import (
    setup_streamlit_page, display_processing_stats, display_answer_card,
    display_source_chunks, display_qa_history, create_download_button,
    show_error_message, show_success_message, show_info_message,
    initialize_session_state, load_environment_variables, validate_environment
)

# Try to import IBM Watsonx, fallback to mock if not available
try:
    from modules.llm_integration import create_watsonx_llm, create_qa_system
    WATSONX_AVAILABLE = True
except Exception as e:
    WATSONX_AVAILABLE = False
    logging.warning(f"IBM Watsonx not available: {e}")
...
class MockLLM:
    """Mock LLM for demonstration when IBM Watsonx is not available"""
...
def display_welcome_message():
    ...
    # Check if we have IBM Watsonx credentials
    env_valid = validate_environment()

    if env_valid and WATSONX_AVAILABLE:
        mode_info = "🚀 **Full AI Mode**: IBM Watsonx AI integration active"
        mode_color = "#d4edda"
    else:
        mode_info = "🎯 **Demo Mode**: Using mock AI responses for demonstration"
        mode_color = "#fff3cd"

    st.markdown(f"""
    <div class="study-container">
        <h1>📚 StudyMate - AI Study Companion</h1>
        <div style="background: {mode_color}; border: 1px solid #ffeaa7; padding: 1rem; border-radius: 5px; margin: 1rem 0;">
            <strong>{mode_info}</strong><br>
            The PDF processing, embeddings, and search functionality are fully operational!
        </div>
...
def process_uploaded_files(uploaded_files: List):
    ...
    try:
        with st.spinner("Processing your PDFs..."):
            ...
            # Initialize LLM (IBM Watsonx or Mock)
            if st.session_state.qa_system is None:
                if validate_environment() and WATSONX_AVAILABLE:
                    try:
                        ...
                        st.session_state.qa_system = create_qa_system(llm)
                        st.session_state.llm_mode = "watsonx"
                    except Exception as e:
                        logger.warning(f"Failed to initialize Watsonx, using mock: {e}")
...
def sidebar_content():
    ...
    st.sidebar.markdown("## 📚 StudyMate")

    # Display current mode
    if hasattr(st.session_state, 'llm_mode'):
        if st.session_state.llm_mode == "watsonx":
            st.sidebar.success("🚀 IBM Watsonx AI Active")
        else:
